...
g_NumBackground			= 34		-- number of available background screens to chose from
g_DLL_MinimumVersion	= 3			-- DLL minimum version to be used
g_Data_MinimumVersion	= 6			-- DataFile minimum version to be used
g_WarningLink			= "http://forums.civfanatics.com/showthread.php?t=472345"

g_AuthorizedModList = {
	g_RED_WWII_ID,							-- R.E.D. WWII Edition
		...
		if g_FirstInitialization then
			...
			if(#mods == 0) then
				...
				for i,v in ipairs(mods) do
					...
					local sToolTip = ""
					if IsModAuthorized (v.ModID) or string.find(displayName, "R.E.D. WWII") then
	...
	for key, modInfo in pairs(unsortedInstalledMods) do
		if modInfo.Enabled then
			if (modInfo.Name) then
				...
				if (not IsModAuthorized ( modInfo.ID ))  and (not string.find(modInfo.Name, "R.E.D. WWII")) then 				
